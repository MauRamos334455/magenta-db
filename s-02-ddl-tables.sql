...
CREATE TABLE ACTIVITY_EVALUATION(
    ...
    CONSTRAINT AE_QUESTION_5_CHK CHECK (QUESTION_5 ~ '^\[(I,)?(F,)?(J,)?(O,)?\]$'),
    CONSTRAINT AE_QUESTION_9_CHK CHECK (QUESTION_9 ~ '^\[(P,)?(H,)?(C,)?(I,)?(O,)?\]$')
)
;
...
CREATE TABLE INSTRUCTOR(
    ...
    CONSTRAINT INSTRUCTOR_KEY_CHK CHECK (KEY ~ '^F04[0-9]{4}[CTSDEF][0-9]{3}R[0-9]{4}$'),
...
CREATE TABLE PARTICIPANT(
    ...
    CONSTRAINT PARTICIPANT_KEY_CHK CHECK (KEY ~ '^F04[0-9]{4}[CTSDEF][0-9]{3}C[0-9]{4}$')
)
;
...
CREATE TABLE PROFESSOR(
    ...
    CONSTRAINT PROFESSOR_PROVENANCE_CHK CHECK (
      (IS_UNAM = FALSE AND PROVENANCE IS NOT NULL AND FACULTY_ID IS NULL) OR 
      (IS_UNAM = TRUE AND PROVENANCE IS NULL AND FACULTY_ID IS NOT NULL)
    ),
...
-- TABLE: PROFESSOR
--
ALTER TABLE PROFESSOR ADD CONSTRAINT PROFESSOR_FACULTY_ID_FK
